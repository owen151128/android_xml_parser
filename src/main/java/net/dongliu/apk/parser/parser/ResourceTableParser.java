...
public class ResourceTableParser {
  ...
  /**
   * By default the data buffer Chunks is buffer little-endian byte order both at runtime and when
   * stored buffer files.
   */
  ...
  private Pair<ResourcePackage, PackageHeader> readPackage(PackageHeader packageHeader) {
    ...
    if (packageHeader.getTypeStrings() > 0) {
      Buffers.position(buffer,
          beginPos + packageHeader.getTypeStrings() - packageHeader.getHeaderSize());
    ...
    if (packageHeader.getKeyStrings() > 0) {
      Buffers.position(buffer,
          beginPos + packageHeader.getKeyStrings() - packageHeader.getHeaderSize());
